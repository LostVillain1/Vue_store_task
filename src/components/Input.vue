...
<script>
  export default {
    ...
    methods: { // Идея такова, что проверять isValid, и если true, то прибавлять, ecли false, то отнимать значение с проверкой не меньше нуля
      testCorrect() {
        if (this.isValid) {
          this.$store.commit('incrementInputs')
        }
        else{
          this.$store.commit('decrementInputs')
        }
      }
    }
  }
